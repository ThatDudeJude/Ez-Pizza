...
from base.models import User, Topping, Regular, Sicillian, Sub, Pasta, Salad, Platter
...
class SitePagesTestCase(TestCase):
    ...
    def test_menu_page(self):
        ...
        self.assertIn(b'Regular', response.content)
        ...
        self.assertIn(b'Sub', response.content)        
        self.assertIn(b'Small', response.content)
        self.assertIn(b'Large', response.content)
        self.assertIn(b'Profile', response.content)
        self.assertIn(b'Special', response.content)
        self.assertIn(b'Cheese', response.content)
        self.assertIn(b'+ Mushrooms', response.content)
        self.assertIn(b'Special', response.content)
        self.assertIn(b'Canadian Bacon', response.content)
        self.assertIn(b'Chicken Parm', response.content)

    def test_empty_shopping_cart(self):
        response = self.client.get("/view/shopping-cart/page/", follow=True)
        self.assertEqual(response.status_code, 200)        
        self.assertIn("base/shopping_list.html", response.templates[0].name)
        self.assertIn(b'No Items in Shopping Cart', response.content)
        self.assertNotIn(b'Place Order', response.content)
    ...
    def test_regular_pizza_shopping_cart(self):
        ...
        response = self.client.post('/shop/add/pizza/', {
        ...
        response = self.client.post('/shop/add-order-item/', 
            data['message']
        ,         
        content_type= 'application/json')

        call_command('dumpdata', 'base.ShoppingCartItem')
        ...
        self.assertIn(b'16.95', response.content)
    ...
    def test_sub_shopping_cart(self):        
        ...
        response = self.client.post('/shop/add/sub/', {
    ...
    def test_pasta_shopping_cart(self):        
        ...
        response = self.client.post('/shop/add/pasta/', {            
            "Baked Ziti w/Chicken": fixed_price,            
            "item": "Pasta"
        }, 
    ...
    def test_salad_shopping_cart(self):        
        ...
        response = self.client.post('/shop/add/salad/', {            
            "salad-Salad w/ Tuna": fixed_price,            
            "item": "Salad"
        }, 
    ...
    def test_platter_shopping_cart(self):        
        ...
        response = self.client.post('/shop/add/platter/', {            
            "small": small_price,
            "large": large_price,
            "type": 'Chicken Parm',
            "item": "Platter"
